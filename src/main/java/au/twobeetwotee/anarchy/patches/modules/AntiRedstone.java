...
public class AntiRedstone extends Module {
    ...
    private void on(BlockRedstoneEvent event) {
        ...
        if (Util.getTPS() < Config.ANTIREDSTONETPS
            ...
            AnarchyPatches.logToAdmins("[BlockRedstoneEvent] Stopped redstone at " + Util.locationToString(event.getBlock().getLocation()));
        }
    ...
    private void on(BlockPistonExtendEvent e) {
        ...
        if (Util.getTPS() < Config.ANTIREDSTONETPS
            ...
            AnarchyPatches.logToAdmins("[BlockPistonExtendEvent] Stopped redstone at " + Util.locationToString(location));
        }
    }
    ...
    private void on(NotePlayEvent e) {
        ...
        if (this.checkChunk(chunk, e.getBlock().getType()) || this.frozenChunks.contains(chunk)) {
            ...
            AnarchyPatches.logToAdmins("[NotePlayEvent] Stopped redstone at " + Util.locationToString(e.getBlock().getLocation()));
        }
    ...
    private boolean checkChunk(Chunk chunk, Material material) {
        for (String string : Config.CHUNKFREEZEMATERIALS) {
            ...
            try {
                ...
                AnarchyPatches.getPlugin().error("Exception: " + e.getMessage());
            }
        }
